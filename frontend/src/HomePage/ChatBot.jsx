...
const ChatBot = () => {
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-8 right-4 sm:right-8 z-[1000]">
      {isOpen ? (
        <div className="w-full sm:w-96 h-[80vh] sm:h-[500px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border-2 border-pink-200 transform transition-all duration-300 max-w-[calc(100vw-2rem)]">
          {/* Header */}
          <div className="bg-[#FB6F92] text-white p-3 sm:p-4 rounded-t-2xl">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-2 sm:space-x-3">
                <div className="p-1 sm:p-2 bg-white/20 rounded-full">
                  <FaSpa className="text-lg sm:text-xl" />
                </div>
                <div>
                  <h2 className="text-lg sm:text-xl font-bold">BloomSkin</h2>
          ...
          {/* Messages */}
          <div className="flex-1 p-3 sm:p-4 overflow-y-auto bg-gray-100">
            {messages.map((message, index) => (
              <div
                key={index}
                className={`mb-3 sm:mb-4 flex ${message.sender === 'bot' ? 'justify-start' : 'justify-end'}`}
              >
                <div
                      ...
                      <div className="mt-1 sm:mt-2 space-y-1 sm:space-y-2">
                        {message.options.map((option, i) => (
                          <button
                            ...
                            className={`flex items-center w-full p-1 sm:p-2 text-left rounded-lg transition-colors ${
                              message.sender === 'bot'
            ...
            {isTyping && (
              <div className="mb-3 sm:mb-4 flex justify-start">
                <div className="max-w-[90%] sm:max-w-[80%] p-2 sm:p-3 rounded-2xl bg-white text-gray-800 rounded-tl-none shadow-sm">
                  <div className="text-xs sm:text-sm flex space-x-1">
          ...
          {/* Input area */}
          <div className="p-2 sm:p-3 border-t border-pink-200 bg-white rounded-b-2xl">
            <div className="flex items-center space-x-1 sm:space-x-2">
              <input
                ...
                className="flex-1 p-2 sm:p-3 border border-pink-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#FB6F92] text-xs sm:text-sm placeholder-pink-400"
              />
              <button
                onClick={handleSendMessage}
                className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-[#FB6F92] text-white rounded-full hover:bg-[#E65A7F] transition-colors focus:outline-none focus:ring-2 focus:ring-[#FB6F92] focus:ring-offset-2"
                disabled={isTyping}
              >
        ...
        <button
          ...
          className="w-12 h-12 sm:w-16 sm:h-16 bg-[#FB6F92] text-white rounded-full shadow-xl flex items-center justify-center hover:scale-110 transition-transform focus:outline-none focus:ring-2 focus:ring-[#FB6F92] focus:ring-offset-2 animate-bounce"
          aria-label="Open chatbot"
          style={{ animationDuration: '2s', animationIterationCount: 'infinite' }}
        >
          <FaSpa className="text-xl sm:text-2xl" />
          <span className="absolute top-0 right-0 w-3 h-3 sm:w-4 sm:h-4 bg-[#FFC2D1] rounded-full border-2 border-white"></span>
        </button>
      )}
    </div>
  );
