...
export const Navbar = ({
  ...
  return (
    (<motion.div
      ...
      className={cn("sticky inset-x-0 top-2 lg:top-8 z-40 w-full", className)}>
    ...
    (<motion.div
      ...
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-gray-900  dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}>
          {hovered === idx && (
            <motion.div
  ...
  return (
    (<a
      ...
      <img
        ...
        width={30}
        height={30} />
      <span className="font-bold text-[#FB6F92] text-xl dark:text-white">Bloom Skin</span>
    </a>)
  );
};
